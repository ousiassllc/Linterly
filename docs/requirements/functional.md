...
| ID | 機能名 | 説明 |
|----|--------|------|
| F-001 | ファイル行数チェック | ファイルごとの行数が上限を超えていないかチェックする |
| F-002 | ディレクトリ行数チェック | ディレクトリ直下のファイルの合計行数が上限を超えていないかチェックする（サブディレクトリ内のファイルは除外） |
| F-003 | 違反レベル判定 | 超過率に基づき error / warn を判定する（閾値はデフォルト 10%、設定で変更可能） |
| F-004 | 行数カウントモード | 全行数（デフォルト）または実行行数（コメント・空行除外）を設定で切替可能 |
...
| F-023 | Python コメント認識 | `#` および `""" """` / `''' '''` をコメントとして認識する |
...
| ID | 機能名 | 説明 |
|----|--------|------|
| F-040 | check コマンド | コード量チェックを実行する |
| F-041 | init コマンド | 設定ファイルを初期化する |
| F-042 | help 表示 | `linterly`（引数なし）でヘルプを表示する |
| F-043 | JSON 出力 | `--format json` で結果を JSON 形式で出力する |
