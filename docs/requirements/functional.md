...
| ID | 機能名 | 説明 |
|----|--------|------|
| F-050 | 更新チェック | 全コマンド実行時に、最新バージョンが利用可能か非同期でチェックし、更新がある場合は stderr に警告を表示する |
| F-051 | 現在バージョン取得 | ldflags で埋め込まれた `Version` を使用する。`"dev"` の場合は `runtime/debug.ReadBuildInfo()` にフォールバックし、`go install` 経由のバージョンを取得する |
